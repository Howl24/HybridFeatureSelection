...
def cho_fitness_function(dna, X, y, features, model, metric, cv, acc_weight):
    X_selected = X[:, features[dna]]
    if X_selected.shape[1] == 0:
        return 0
    ...
    acc = evaluate_metric(metric, y, y_pred)
    n_feats = len(features[dna])
    return acc_weight * acc - (1 - acc_weight) * n_feats / len(features)
...
class GeneticAlgorithm(object):
    ...
    def __init__(self, population_size=30, mutation_rate=0.1,
    ...
    def run(self, n_iter, plot=False):
        ...
        return max(selected_individuals, key=lambda i: i.fitness), selected_individuals
